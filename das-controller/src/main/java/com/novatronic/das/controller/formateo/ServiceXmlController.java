 ...
 *
 */
public class ServiceXmlController implements ServiceController{
	private final static Logger log = LoggerFactory.getLogger(ServiceXmlController.class.getName());

	@Override
	public List<Service> obtener() {
	...
	public Respuesta insertar(Service t) {
		try { 
			XmlDAOFactory.getInstance().getServiceDAO().create(t);
			log.debug("Insert: [" + t + "]");
	...
	public Respuesta actualizar(Service t) {
		try { 
			XmlDAOFactory.getInstance().getServiceDAO().update(t);
			log.debug("Update: [" + t + "]");
