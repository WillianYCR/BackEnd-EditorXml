 ...
 *
 */
public class RouteXmlController implements RouteController{
	private final static Logger log = LoggerFactory.getLogger(RouteXmlController.class.getName());

	@Override
	public List<Route> obtener() {
	...
	public Respuesta insertar(Route t) {
		try { 
			XmlDAOFactory.getInstance().getRouteDAO().create(t);
			log.debug("Insert: [" + t + "]");
	...
	public Respuesta actualizar(Route t) {
		try { 
			XmlDAOFactory.getInstance().getRouteDAO().update(t);
			log.debug("Update: [" + t + "]");
